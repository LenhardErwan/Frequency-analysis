 ...
 * @brief 
 * 
 * @param a Analyse concernée
 * @param graphene graphène à incrémter
 * @param semaphore verouiller et dévérouiller l'accès en ecriture de l'Analyse
 * @return true si un graphène à été incrémenter
 * @return false sinon
 */
...
int main(int argc, char *argv[]) {
    if (argc != 2) { // verification de la présence des arguments
        ...
        try {
            ...
            //On met toutes les lignes dans des buffers
            while(std::getline(fic,line) ) {
            ...
            //Créer tous les threads pour l'analyse
            for (size_t i = 0; i < NB_THREADS; i++) {
            ...
            //Calcul de toutes les fréquence
            aLetter->calcFreq();
            aDigramme->calcFreq();
            aTrigramme->calcFreq();
